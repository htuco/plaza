...
- **Pattern:** A short name for the duplicated logic
- **Locations:** Every `path/to/file.ts:line` where it appears, with the code snippet
- **Suggested utility:** The extracted function implementation (with a sensible file path, e.g. `src/lib/utils/...`)
- **Refactored call sites:** How each location would call the new utility

End with a summary count of findings per impact level.
